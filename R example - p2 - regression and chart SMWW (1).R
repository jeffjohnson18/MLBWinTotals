...
library(Lahman)   # baseball-specific library
# install.packages('dplyr')
library(dplyr)    # manipulate data
# install.packages('ggplot2')
library(ggplot2)  # visualization
# install.packages('caret')
library(caret)    # measures accuracy of model
...
head(teams)
## see there is a new column called runs_game

teams_year <- teams %>%
  group_by(yearID) %>%
  summarize(mean_runs = mean(runs_game, na.rm=TRUE))

## removes blanks with na.rm=TRUE

teams_year %>%
  ggplot(aes(x=yearID, y=mean_runs)) +
  geom_line() +
  geom_point() +
  ggtitle('hello SMWW MLB Runs by Year')
